...
Task Build-Solution -Depends Clean, Generate-VersionInfo {
	Exec { msbuild "$baseDir\jQuery.sln" /verbosity:minimal /p:"Configuration=$configuration" }
}

Task Build-NuGetPackages -Depends Determine-Version, Build-Solution {
	$config = [xml](Get-Content $baseDir\jQuery\packages.config)
...
"@ | Out-File -Encoding UTF8 "$outDir\jQuery.nuspec"
	...
	Exec { & "$buildtoolsDir\nuget.exe" pack "$outDir\jQuery.nuspec" -NoPackageAnalysis -OutputDirectory "$outDir" }
}
...
Task Generate-VersionInfo -Depends Determine-Version {
	Generate-VersionFile -Path "$baseDir\jQuery\Properties\Version.cs" -Version $script:Version
}
